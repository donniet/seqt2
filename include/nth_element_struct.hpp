...
inline void nth_element_struct(Iterator first,
                        Iterator nth,
                        Iterator last,
                        Compare compare,
                        command_queue &queue = system::default_queue())
{
    ...
    while(1)
    {
        ...
        auto less_than_value = ::boost::compute::bind(compare, _1, value);
        auto greater_or_equal_than_value = ::boost::compute::not1(less_than_value);
        // TODO: no _2 in compare
        auto greater_than_value = ::boost::compute::bind(compare, value, _1);
        auto less_or_equal_than_value = ::boost::compute::not1(greater_than_value);


        Iterator new_nth = partition(
            first, last, less_than_value, queue
        );

        Iterator old_nth = find_if(new_nth, last, less_or_equal_than_value, queue);
                        ...
                        command_queue &queue = system::default_queue())
{
    ...
    return nth_element_struct(first, nth, last, less_than, queue);
}
